...
local equippedCount    = 1 -- demo: 1 item equipped
...
function Shop:load()
    ...
    local ok, s = pcall(love.graphics.newShader, "assets/shaders/shop_bg.glsl")
    if ok then bgShader = s end
    bgCanvas = love.graphics.newCanvas(love.graphics.getDimensions())
...
function Shop:draw()
    ...
    -- ── Background with crosshatch shader ──
    if bgShader and bgCanvas then
    ...
    for i, eq in ipairs(shopEquipment) do
        ...
        local canBuy = playerGold >= eq.price and equippedCount < maxEquipSlots
    ...
    local lvlText = "Level " .. playerLevel
        ...
        if btn.enabled and pointInRect(x, y, btn.x, btn.y, btn.w, btn.h) then
            ...
            elseif btn.id == "level_up" and playerGold >= levelUpCost then
                ...
                if playerXP >= xpToNext then
                    ...
                    print("[Shop] Leveled up to " .. playerLevel .. "!")
                else
            ...
            elseif btn.id:match("buy_eq_(%d+)") then
                ...
                if eq and playerGold >= eq.price and equippedCount < maxEquipSlots then
